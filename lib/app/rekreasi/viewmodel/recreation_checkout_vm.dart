...
class RecreationCheckoutVM extends BaseViewModel {
  ...
  onSubmit(
    ...
    final totalTickets = items.fold(0, (sum, e) => sum + e.quantity);

    showLoading(context);
    RecreationService.checkoutRecreation(context, data: {
      "service": "recreation",
      "payment": "xendit",
      "packages": items.map((e) => e.package.id.toString()).toList(),
      "quantities": items.map((e) => e.quantity.toString()).toList(),
      "point": usePoint ? '1' : '0',
      "total_ticket": totalTickets.toString(),
      "status": "pending"
    }).then((value) async {
      Navigator.pop(context);
